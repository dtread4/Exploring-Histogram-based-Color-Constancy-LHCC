...
# training_utility.py - Trains a CNN from the start, creating a new model
...
def train_from_start(parameters, output_dir, valid_fold_number, preloaded_histograms):
    """
    ...
    network, optimizer, scheduler = create_network(parameters)
    ...
    # Save the minimum error and the epoch it occurred at for various metrics and the last epoch's error
    save_lowest_epoch_error(test_loss_metrics, output_dir_fold, parameters.epochs, param_count)

    # Update histograms if training with static histograms and if more than one cross validation fold
    # Only do this if the preloaded histograms dictionary is None (not yet initialized)
    if parameters.projection_type == "STATIC" and preloaded_histograms is None:
...
def main(argv):
    ...
    for valid_fold_number in range(parameters.num_folds):
        ...
        preloaded_histograms, metrics = train_from_start(parameters, output_dir, valid_fold_number + 1,
...
if __name__ == "__main__":
    ...
    Command line arguments needed:
        ...
        2. Output directory for models (will save a checkpoint every n epochs, as defined in the parameters file)
    """    
    main(sys.argv)
