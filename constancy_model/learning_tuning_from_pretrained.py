...
# learning_tuning_from_pretrained.py - Compares different ways of handling the leaning parameters when training the CNN
# Uses a pre-trained network and optimizer, rather than a new one
...
from constancy_model.load_and_train import load_and_train, check_and_set_directory
import numpy as np


def run_learning_tuning(output_dir, default_parameters, learning_tuning_parameters, valid_fold_number, model_path):
            ...
            for optimizer in learning_tuning_parameters.optimizers:
                ...
                for momentum in momentum_vals:
                    for batch_size in learning_tuning_parameters.batch_sizes:
                        ...
                        preloaded_histograms, curr_loss_metrics = load_and_train(parameters_tuning,
                                                                                 model_path,
                                                                                 full_output_dir,
                                                                                 valid_fold_number,
...
def main(argv):
    ...
    if len(argv) < 6:
        ...
        print(" - Pre-trained optimizer path")


    output_dir = argv[1]
    parameter_file_path = argv[2]
    learning_tuning_file_path = argv[3]
    valid_fold_number = int(argv[4])
    model_path = argv[5]
    ...
    run_learning_tuning(output_dir, parameters, learning_tuning_file_path, valid_fold_number, model_path)
...
if __name__ == "__main__":
    ...
    Command line arguments needed:
        ...
        4. Validation fold number
        5. Pre-trained model path
    """
    main(sys.argv)
